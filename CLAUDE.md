...
| `src/EntryCreatedOverlay.tsx` | Swipe-to-confirm **success animation** — a green "¡Entrada creada!" card that performs a macOS-style **genie flight** into the "Mis entradas" tab (spring y with anticipation lift + delayed x, position-driven squash/stretch, shrink envelope, borderRadius morph, rotation from x-velocity). Ported from github.com/Jate099/entry-success-animation. `onCatch` bumps the tab icon at ~88%; `onDone` triggers the count badge + "¿Reusar?" prompt. All timing in its `cfg`. |
